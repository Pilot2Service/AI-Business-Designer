# Strategic Thinking

Systeeminen, hypoteesivetoinen strateginen ajattelu (Liedtka 1998, McKinsey MECE / issue tree).

## Skillit tässä pakissa

| Skilli | Kuvaus |
|---|---|
| `systems-mapping` | Kartoittaa arvonluonnin kokonaisjärjestelmän ja sen riippuvuudet ennen ratkaisun muotoilua. |
| `strategic-intent-framing` | Muotoilee kirkkaan strategisen tarkoituksen, joka fokusoi energian ja karsii häiriöt. |
| `hypothesis-driven-strategy` | Jäsentää strategisen ongelman MECE-periaatteella ja issue tree -rakenteella, testaa hypoteeseja datalla. |
| `scenario-and-foresight` | Rakentaa vaihtoehtoisia tulevaisuusskenaarioita ja testaa strategian kestävyyttä niitä vasten. |
| `strategic-options-evaluation` | Vertailee strategisia vaihtoehtoja jäsennellysti (vaikutus/vaiva, riski, resurssit) päätöksenteon tueksi. |

Kaikki `maturity: scaffold` — ks. `../skills_index.json` kypsyystilalle (frontmatterissa
ei seurata kypsyyttä, ks. `../meta/frontmatter_schema.md`).

## Ankkurointi

- Liedtka (1998) — hypothesis-driven
- Liedtka (1998) — intelligent opportunism
- Liedtka (1998) — intent-focused
- Liedtka (1998) — systeeminäkökulma (systems perspective)
- Liedtka (1998) — thinking in time
- McKinsey — MECE / issue tree / hypoteesivetoinen ongelmanratkaisu
- McKinsey — structured problem solving

## Rakenne

```
CLAUDE.md                    pakin jaetut suojaukset (lue aina ensin)
skills/<skill-id>/SKILL.md   yksittäinen skilli (name + description -frontmatter)
references/                  taustamateriaali, lähteet, omat mallit (täydennettävä)
```

Katso `../meta/maturity_levels.md` kypsyystasojen selityksille ja
`../AGENT_GUIDE.md` sille, miten agentin tulee lukea ja painottaa tämän pakin sisältöä.
